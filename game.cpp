...
int main() {
  ...
  while (num_of_wrong_guesses < MAX_TRIES) {
     cout << hangmanArt[num_of_wrong_guesses] << endl;
    cout << "\n\n" << unknown;
    cout << "\n\nGuess a letter: ";
    cin >> letter;
...
int letterFill(char guess, string secretword, string & guessword) {
  ...
  char guessUpper = toupper(guess);  // Convert guessed letter to uppercase

  for (i = 0; i < len; i++) {
    // Did we already match this letter in a previous guess?
    if (toupper(guessword[i]) == guessUpper)
      return 0;
    // Is the guess in the secret word?
    if (toupper(secretword[i]) == guessUpper) {
